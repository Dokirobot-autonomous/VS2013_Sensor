...
public:
	...
	void readEnvDescriptor(std::string filepath){
		...
		int fnum = 1;
		while (true){
			std::string filename = filepath + "descriptor/descriptor_" + std::to_string(fnum) + "th.csv";
			...
			for (int i = 0; i < v.size(); i++){
				cv::Mat mat(v[i], true);
				mat = mat.t();
				descriptors_tmp.push_back(mat);
			}
			env_descriptors.push_back(descriptors_tmp);
			std::cout << "descriptor: " << fnum << std::endl;
			fnum++;
			//if (fnum > 2){
			//	break;
			//}
	...
	void setEnvDescriptor(){
		...
		for (int i = 0; i < env_img.size(); i++){
			...
			//画像の特徴点における特徴量を抽出
			cv::Mat desc;
			extractor.compute(env_img[i], env_keypoints[i], desc);
	...
	void readEnvImgSURFAvi(std::string filename){
	...
	void setMeasBofSift(Position<T> estimated_position)
	{
		...
		matcher->match(descriptor, env_centroid, dmatch, masks);
	...
	void setMeasBofSift1(Position<T> est_pos)
	{
		...
		for (int i = 0; i < dot_product.cols; i++)
		{
			...
			//if (dot_product(0, i) > BOF_TH_SIM && distance < ENV_IMG_TH)	//	前ステップの推定位置とimg_posで移動半径内の画像
			if (distance < ENV_IMG_TH)	//	前ステップの推定位置とimg_posで移動半径内の画像
			{
				sim_img_idx.push_back(i);
			}
		}
	...
	/* */
	template<typename T>
	void calcSimilarityBetweenImages(const std::vector<Position<T>>& particles){
	...
	double getLikelihoodBofSift(const Position<T> particle)
	{
		...
		for (int idx : sim_img_idx)
		{
			...
			out *= std::exp(-std::pow(dis, 2) / (2.0 * std::pow(sgm, 2)));	//	正規分布
		}
		return out;
	}

	template<typename T>
	double getLikelihoodSURF(const Position<T> particle)
	{
		double out = 1.0;
		...
		for (int i = 0; i < imgForCalcLikeAll_positioin.size(); i++){
			...
			if (dis < ENV_IMG_TH){
				...
				if (xy < 0) {
					std::cout << "In Omni xy<0" << std::endl;
					exit(0);
				}
		...
		if (out < 0) {
			std::cout << "In Omni out<0" << std::endl;
			exit(0);
		}
		return out;
	}
