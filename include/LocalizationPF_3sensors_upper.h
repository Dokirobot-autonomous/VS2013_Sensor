...
public:
	...
	inline int minAllMeasurementData() {
		int min = std::min({
			all_meas_odometry.size(), all_meas_time.size(), all_meas_lrf_u.size(),
...
public:
	...
	void writeUseSensorAll(std::string ofpath)
	{
		...
		ofs << "step,time,s,lrf_u,cmr,gps" << std::endl;
	...
	void writeOutputParticleAll(std::string ofpath) {
		...
		ofs << "step,time,s,x,y,rad,fusion,lrf_u,cmr,gps" << std::endl;
	...
	void writeOutputStatParticleAll(std::string ofpath) {
		...
		ofs << "step,time,s,x,y,rad,lrf_u,cmr,gps" << std::endl;
	...
	{
		std::string filename = ofpath + "Data/gitignore/particle/lrf_u/particle_" + std::to_string(estimated_position.size()) + "_" + esti_time.filenameStr() + ".csv";
		std::ofstream ofs(filename, std::ios_base::out);
		if (ofs.fail()) {
			writeError(filename);
		}
	...
	{
		std::string filename = ofpath + "Data/gitignore/stat_particle/lrf_u/particle_" + std::to_string(estimated_position.size()) + "_" + esti_time.filenameStr() + ".csv";
		std::ofstream ofs(filename, std::ios_base::out);
		if (ofs.fail()) writeError(filename);
		ofs << "x,y,rad,weight" << std::endl;
