...
TCHAR buf1[1024] = _T("COM6");
LPCWSTR buf2 = buf1;
/* ここまで */

/* LRF */
#define LRF_UPPER_COM_PORT "COM19"
#define LRF_UNDER_COM_PORT "COM7"
/* ここまで */
...
int main()
{
	...
	const char connect_device2[] = LRF_UNDER_COM_PORT; // 要設定
	...
	ret = urg_start_measurement(&urg2, URG_DISTANCE, 1, 0); // LRFデータ取得（1回やっておいた方が良い）
	...
	/* 制御コマンド変数定義 */
	char end; // 計測終了
	bool lrf_upper_start_ = false;
	bool lrf_under_start_ = false;
	...
	std::thread th_cont([&end, &lrf_upper_start_, &lrf_under_start_, &cmr_start_, &atr_start_, &urg1, &length_data1, odo_ini, &sp_gps, &step]{
		cv::namedWindow("Press 'e' to end");
		clock_t lap1 = clock();
		clock_t lap2;
		while (end != 'e'){
			//if (lrf_start_ || cmr_start_ || atr_start_) continue;	// 他のセンサの計測が終わっていない場合、待つ
			if (lrf_upper_start_ || cmr_start_) continue;	// 他のセンサの計測が終わっていない場合、待つ
			lap2 = clock();
			int rest = UPDATE_TIME - (int)(lap2 - lap1);
			if (rest > 0)	end = cv::waitKey(rest);
			lrf_upper_start_ = true;
			lrf_under_start_ = true;
			cmr_start_ = true;
			atr_start_ = true;
			...
			//LRF_UPPER取得
			if (!lrf_upper_start_) continue; // lrf_start_=tureとなるまで待つ
	...
	//std::thread th_lrf1([&end, &lrf_under_start_, &step, &urg1, &length_data1, &cmr_start_, &cap, &atr_start_, &atr]{
	//	while (end != 'e'){
	//		if (!lrf_under_start_) continue; // lrf_start_=tureとなるまで待つ
	//		clock_t start = clock();
	//		urg_start_measurement(&urg1, URG_DISTANCE, 1, 0); // LRFデータ取得
	//		int ret = urg_get_distance(&urg1, length_data1, NULL); // データ更新と同時にデータサイズ取得
	//		// LRFデータをファイル出力
	//		char filename[256];
	//		sprintf_s(filename, "./output/lrf_under/lrf_no%d_%dth.csv", NO, step);
	//		std::ofstream ofs_lrf(filename, ios_base::out);
	//		if (ofs_lrf.fail())
	//		{
	//			writeError(filename, false);
	...
	/* LRFの計測 */
	std::thread th_lrf2([&end, &lrf_upper_start_, &lrf_under_start_, &step, &urg2, &length_data2, &cmr_start_, &cap, &atr_start_, &atr]{
		atr.start();
		while (end != 'e'){
